...
\posveta{Ovaj rad posvećujem porodici}
...
Replikacija podrazumeva da se više kopija podataka čuva na različitim čvorovima, odnosno replikama. Broj replika svakog fragmenta koji se replikuje predstavlja faktor replikacije. Ukoliko neki od čvorova postane nedostupan, klijentski zahtevi mogu biti obrađeni na nekom od preostalih čvorova. To sisteme koji koriste replikaciju čini pouzdanijim. Kako bi svaka replika bila ažurna neophodno je da se svaka izmena podataka propagira do svakog čvora. Najpoznatiji model i najjednostavniji po kojem se to realizuje je \textit{master-slave}. Postoje i alternative: kao što su \textit{multimaster}, konsenzus kvoruma itd. U \textit{master-slave} modelu jedna od replika izabrana je da bude master replika i svaka izmena podataka mora ići kroz nju. Dalje, ona izmene propagira do ostalih zavisnih (\textit{slave}) replika. Čitanje podataka se sa druge strane, može 
...
U svrhu demonstracije i merenja performansi HBase-a u distribuiranom okruženju biće iskorišćeni testovi iz testiranja OLTP i OLAP  scenarija. Testiranje se izvršava na jednom fizičkom čvoru. Koristiće se HBase klaster sa  jednim master serverom, dva region servera, zookeeper servisom, jednim HDFS master čvorom i dva HDFS čvora podataka. Svi oni će se izvršavati kao zasebni procesi. Usled složenosti implementacije distribuiranog relacionog sistema (u ovom slučaju PostgreSQL-a), za distribuirano okruženje primer je dat samo za HBase.
...
U ovom slučaju testiranje se izvršava na fizičkom čvoru (računaru) sa 10 procesorskih jezgara i ranije instaliranim HBase-om i Hadoop-om. 

Priprema okruženja podrazumeva najpre pokretanje HDFS klastera, koji se sastoji iz master čvora i dva HDFS čvora podataka, sa postavljenim faktorom replikacije blokova:

\begin{lstlisting}[title={start-hadoop.sh - Komande za pokretanje HDFS komponenti},captionpos=b]
#!/bin/bash
$HADOOP_HOME/bin/hdfs namenode -format
$HADOOP_HOME/sbin/start-dfs.sh
hdfs --config $HADOOP_HOME/etc/datanode1/ datanode  \
    -D dfs.datanode.data.dir=/home/lukadj/datanode2 &
$HADOOP_HOME/sbin/start-yarn.sh
\end{lstlisting}

\begin{lstlisting}[title={hdfs-site.xml - Konfiguracija prvog HDFS čvora sa faktorom replikacije 2},captionpos=b]
<configuration>
      <property>
     ...
     <property>
        ...
        <value>/home/lukadj/datanode1</value>
    	</property>
    <property>
</configuration>
\end{lstlisting}

\begin{lstlisting}[title={hdfs-site.xml - Konfiguracija drugog  HDFS čvora sa faktorom replikacije 2},captionpos=b]
<configuration>
       <property>
         <name>dfs.replication</name>
         <value>2</value>
     ...
     <property>
        <name>dfs.datanode.data.dir</name>
        <value>/home/lukadj/datanode2</value>
     </property>   
</configuration>
\end{lstlisting}


Nakon toga pokreće se HBase klaster koji se sastoji iz jednog master servera, dva region servera i zookeeper servisa. HBase je konfigurisan tako da koristi ranije pokrenut HDFS klaster.

\begin{lstlisting}[title={start-hbase.sh - Komanda za pokretanje HBase-a},captionpos=b]
#!/bin/bash
$HBASE_HOME/bin/start-hbase.sh
HBASE_CONF_DIR=/home/lukadj/hbase-regionserver1 \
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=1\textwidth]{hbase-master.png}
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=1\textwidth]{datanode.png}
...
Analiza rezultata testiranja u distribuiranom okruženju obuhvata uporednu analizu sa nedistribuiranim HBase-om, opis fragmentacije, replikacije i organizacije podataka HBase-a na HDFS klasteru. 

Pokrenuti distribuirani HBase klaster pokazuje bolje rezultate nego nedistribuirani HBase koji podatke skladišti na lokalnom fajl sistemu. Jedan od razloga za to je integracija HBase baze podataka sa HDFS ekosistemom \cite{hbaseGuide}. Glavni razlog ipak jeste paralelizacija obrade zahteva pomoću dva region servera. S obzirom da su podaci podeljeni na regione koji su dodeljeni odgovarajućim region serverima, ukoliko više klijenata našeg testa pristupa regionima koji su smešteni u više region servera, tada se postiže veća iskorišćenost procesora.
Merenje iskorišćenosti procesora (\textit{CPU}\%) u nedistribuiranom i distribuiranom okruženju sa dva region servera dalo je prednost distribuiranom, Slike 5.4 i 5.5. 
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.8\textwidth]{distribuirano-cpu2.png}
  \caption{Iskorišćenost procesora u distribuiranom okruženju}
  \label{fig:grafikon}
\end{figure}

\pagebreak 
Ograničenje ove studije je korišćenje jednog fizičkog čvora. Sa povećanjem broja  fizičkih čvorova potencijalno bi se postigla veća iskorišćenost odvojenih procesora, a samim tim i bolje performanse, što može biti predmet daljih istraživanja. Osim toga, povećavanjem broja fizičkih čvorova postigla bi se veća pouzdanost klastera, bolja dostupnost i brže čitanje podataka \cite{ColumnarOriented}.  
...
Lista regiona, region serveri kojima pripadaju, njihovi opsezi, kao i veličine mogu se videti na  primeru tabele orderitem, na Slici 5.7.
